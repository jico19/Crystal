...
- **Code Location:** `apps/api/src/modules/documents/doc-ocr.agent.ts`
...
- **Trigger:** Event-driven trigger in the Unified API `documents` module upon document upload confirmation (`POST /api/v1/documents/:id/confirm`).
- **Capabilities & Workflow:**
  ...
  3. Pre-populates document metadata fields in PostgreSQL (`expiration_date`, `ocr_extracted_data`).
...
- **Code Location:** `apps/api/src/modules/credentials/compliance.agent.ts`
...
- **Trigger:** In-process `node-cron` schedule (`0 0 * * *` at 00:00 UTC) executed inside the Express API process.
- **Capabilities & Workflow:**
  1. Queries all active caregivers in `caregiver_profiles` and scans their related `credentials` and `caregiver_documents`.
...
- **Code Location:** `apps/api/src/modules/authorizations/authorization-cron.job.ts`
...
- **Trigger:** In-process `node-cron` schedule (`0 1 * * *` at 01:00 UTC) executed inside the Express API process.
- **Capabilities & Workflow:**
  1. Scans `client_authorizations` table across all active clients.
...
- **Code Location:** `apps/api/src/modules/training/training-cert.agent.ts`
...
- **Trigger:** Event-driven on `POST /api/v1/training/courses/:id/quiz`.
- **Capabilities & Workflow:**
  1. Grades submitted quiz responses against course answer keys and passing score thresholds (typically $\ge 80\%$).
  2. Updates `caregiver_training_progress` table (`passed` / `failed`, score percentage).
  3. If passed:
     ...
     - Uploads certificate to private S3 bucket and records link in `caregiver_training_progress`.
...
- **Code Location:** `apps/api/src/modules/notifications/notification-dispatcher.job.ts`
...
- **Trigger:** In-process `node-cron` schedule (`* * * * *` every minute) polling `notification_queue`.
- **HIPAA Guardrails:**
...
- State-specific Vite SPAs (`apps/georgia`, `apps/indiana`, future `apps/florida`) are responsible for UI, client-side routing (React Router v7), presentation, and form capture.
- **Do not duplicate business rules, validation logic, credential checks, or authorization evaluation inside state frontend apps.** Centralize all business rules in the Unified API (`apps/api`).

### Rule 4: No Bypassing the Unified API
- Frontend state applications must never connect directly to the database or invoke third-party transactional APIs (SES, Twilio, DocuSign) directly. All requests must route through the Express.js Unified API via standard `fetch` endpoints (`/api/v1/...`).

### Rule 5: Never Rely Solely on Frontend Authorization
- Frontend UI guards and hidden buttons are strictly for user experience.
- Every API endpoint must independently verify JWT authentication, user role, and state/organization permissions via Express middleware (`verifyJWT`, `requireRole`, `requireOrg`).
...
- When Georgia and Indiana require different behaviors (e.g. licensing disclosures, office addresses, branding colors, required training hours), use database-driven organization configuration (`organizations.branding_theme` / JSONB settings) or shared constants rather than duplicating entire code files.
...
- Always upload to private S3/Storage buckets and deliver file access exclusively through short-lived (15-minute) cryptographically signed URLs generated on-demand by the Unified API.
...
- Standard stack: **Vite 8 + React 19 (SPA per state)**, **Tailwind CSS v4 + shadcn/ui**, **Express.js v5 (Unified API)**, **PostgreSQL 15+ with RLS**, **Supabase Storage / AWS S3**, **Amazon SES**, **Twilio**.

### Rule 10: Mandatory Database Schema Versioning (`db_schema/`)
- **Every time there are changes to the database schema (DDL, tables, columns, indexes, RLS policies, or migrations), the agent must store the full updated schema in the `db_schema` folder.**
- File naming convention:
  ```text
  db_schema/db_schema_<date>_v<version>.sql
  ```
  *(e.g., `db_schema/db_schema_20260901_v1.sql`, `db_schema/db_schema_20260901_v2.sql`)*
...
| :--- | :--- | :--- |
| **State Vite SPAs**<br/>(`apps/georgia`, `apps/indiana`) | • UI Pages, components, & React Router v7 routes<br/>• State-specific marketing content & branding<br/>• Form inputs, client-side maskings, React Hook Form<br/>• UI state machines (idle, submitting, error)<br/>• Theme overrides (Deep Teal vs Navy) | • Direct database queries or SQL execution<br/>• Authentication credential hashing & JWT signing<br/>• Credential expiration calculations<br/>• Direct SES/Twilio/DocuSign API keys<br/>• Duplicated backend APIs or SSR handlers |
| **Unified Crystal API**<br/>(`apps/api`) | • Express.js v5 modular domain monolith<br/>• Authentication & JWT session token lifecycle<br/>• RBAC (`verifyJWT`, `requireRole`, `requireOrg`)<br/>• Server-side Zod payload validation<br/>• Quiz evaluation & PDF certificate generation<br/>• Multi-channel notification queueing & in-process crons<br/>• Secure presigned S3 upload/download URLs<br/>• Third-party cloud integration adapters (SES, Twilio, DocuSign) | • UI components or HTML rendering<br/>• State-specific duplicated endpoints (e.g. `/api/georgia/...`)<br/>• Microservice network hops<br/>• Exposing raw unverified database access |
...
When building, testing, and maintaining the platform codebase using task-based specifications (Phase 1 Database, Phase 2 Express API, Phase 3 Vite Frontend), the following specialized subagents are utilized:
...
graph LR
    ...
    subgraph DevSubagents["Specialized Development Subagents"]
        FrontendAgent["Frontend & UI Subagent<br/>(Vite 8, React 19, apps/georgia, apps/indiana)"]
        BackendAgent["Backend & API Subagent<br/>(Express.js v5, apps/api, PostgreSQL, RLS)"]
        IntegrationAgent["Integrations Subagent<br/>(SES, Twilio, DocuSign, S3)"]
        QASubagent["QA & Security Subagent<br/>(Vitest, Playwright, RLS Audit)"]
    end
...
- **Focus:** Phase 3 task execution — Vite 8 + React 19 SPAs (`apps/georgia`, `apps/indiana`), React Router v7 routes, shared `@crystal/ui` components, accessible forms with `react-hook-form` + `@crystal/validation`, responsive dashboard layouts.
- **Tools:** TypeScript, Vite, React 19, Tailwind CSS v4, shadcn/ui, Lucide Icons.

### 5.2 Backend & API Subagent (`subagent-backend`)
- **Focus:** Phase 1 & 2 task execution — Unified Crystal API (`apps/api`), Express.js v5 routers/controllers/services, PostgreSQL DDL migrations, Row-Level Security (RLS) policies, and in-process `node-cron` background jobs.
- **Tools:** TypeScript, Node.js 22 LTS, Express 5, Drizzle ORM / `pg`, Vitest, Supertest.

### 5.3 API & Integration Subagent (`subagent-integration`)
- **Focus:** Implementing resilient integration adapters (`apps/api/src/integrations/` for Amazon SES, Twilio SMS, DocuSign/SignWell e-signatures, Cloudflare Stream, AWS S3 presigned URLs), webhook signature verification, and notification queue dispatchers.
- **Tools:** Node.js, TypeScript, AWS SDK v3, Axios, `node-cron`.

### 5.4 QA & Security Audit Subagent (`subagent-qa-security`)
- **Focus:** End-to-end multi-state journey testing (Playwright), RLS isolation test matrices (verifying GA users cannot query IN records), API route security verification, and zero-PHI notification audit.
- **Tools:** Playwright, Vitest, ESLint security rules.
...
| :--- | :--- | :--- |
| `Agent-DocOCR` | Blurry image / unparseable text | Flags document as `under_review`, assigns task to coordinator review queue without blocking applicant progress. |
| `Agent-Compliance` | Database timeout / lock contention | Automatic retry with exponential backoff (3 attempts); alerts Super Admin via Sentry/CloudWatch if unresolved. |
| `Agent-TrainingCert` | PDF generation failure | Enqueues retry task; marks status as `cert_pending_generation`; notifies user certificate will be ready shortly. |
| `Agent-Notifier` | SES / Twilio rate limit or bounce | Exponential backoff retry (up to 3 attempts); updates queue status to `failed` and alerts coordinator dashboard. |
