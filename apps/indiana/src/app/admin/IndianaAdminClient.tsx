...
export function IndianaAdminClient() {
    ...
    const fetchAdminData = async () => {
      try {
        const [kpiRes, queueRes] = await Promise.all([
          fetch(`${apiBaseUrl}/api/v1/admin/kpis?state_code=IN`),
          fetch(`${apiBaseUrl}/api/v1/admin/queues?state_code=IN`),
        ]);
        ...
        console.warn('Failed to load Indiana admin metrics:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading || !metrics) {
    return (
      <div className="py-24 text-center text-slate-500">
        ...
        <p className="text-sm font-semibold">Loading Indiana State Command Center...</p>
      </div>
    );
  }
