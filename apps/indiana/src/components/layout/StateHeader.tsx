        ...
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
          ...
          <div className="flex items-center space-x-4">
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-blue-400/90 font-medium">{st.badge}</div>
                        </div>
                        {isCurrent && (
                          <span className="text-[10px] font-bold text-blue-400 px-1.5 py-0.5 rounded bg-blue-950 border border-blue-800">
                            Active
                          </span>
                        )}
                      </a>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 font-semibold text-blue-400 hover:text-blue-300 text-xs transition-colors"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center space-x-3.5 group">
          <div className="relative">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-blue-500/20 to-blue-600/10 text-blue-400 border border-blue-500/30 group-hover:border-blue-400 transition-all shadow-sm">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="font-bold text-lg text-white block leading-tight tracking-tight group-hover:text-blue-300 transition-colors">
                {org.name}
              </span>
        ...
        <nav className="hidden md:flex items-center space-x-7">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-blue-500/20 text-blue-300 font-bold border border-blue-500/30 animate-pulse">
                  {item.badge}
                </span>
              )}
            </NavLink>
        ...
        <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    {currentUser && (
                      <span className="text-[10px] text-slate-400">
                        Role: <strong className="text-blue-400 uppercase">{currentUser.role?.replace('_', ' ')}</strong>
                      </span>
                    )}
                  </div>
                  <span className="text-[10px] text-blue-400 font-medium">HIPAA Compliant</span>
                </div>
                ...
                {portalItems.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <div className="text-xs font-semibold text-white group-hover:text-blue-300 flex items-center gap-1">
                          <span>{p.label}</span>
                ...
                <div className="pt-2 border-t border-slate-800/80">
                  {currentUser ? (
                    ...
                    <Link
                      to="/auth/login"
                      onClick={() => setPortalDropdownOpen(false)}
                      className="block text-center py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-sm transition-colors"
                    >
          ...
          <Link
            to="/contact"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold tracking-wide text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 transition-all shadow-md shadow-blue-900/30 cursor-pointer"
          >
        ...
        <div className="md:hidden border-t border-slate-800 bg-slate-950 px-4 pt-4 pb-8 space-y-4 animate-in slide-in-from-top-4 duration-200">
              ...
              {portalItems.map((p) => {
                ...
                return (
                  <Link
                    ...
                    to={p.path}
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex flex-col items-start p-2.5 rounded-xl bg-slate-950 border border-slate-800/80 hover:border-blue-500/40 text-left transition-colors"
                  >
          ...
          <div className="pt-2 space-y-2">
            <Link
              to="/contact"
              onClick={() => setMobileMenuOpen(false)}
              className="block text-center w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 text-sm font-semibold text-white shadow-lg"
            >
