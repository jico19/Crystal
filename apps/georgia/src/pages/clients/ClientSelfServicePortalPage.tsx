...
export const ClientSelfServicePortalPage: React.FC = () => {
    ...
    try {
      ...
      if (Array.isArray(clientList) && clientList.length > 0) {
        ...
        setClient({
          ...
          dob: '1945-06-15',
          gender: 'Female',
          medicaid_id: 'GA-MED-994821',
          status: 'active',
          service_address: {
            street: '450 Northside Dr NW, Apt 3B',
            city: 'Atlanta',
            state: 'GA',
            zip: '30318',
          },
          ...
          payer_details: {
            payer_type: 'Medicaid Waiver (CCSP / SOURCE)',
            program_name: 'Elderly & Disabled Waiver Program',
            case_manager_name: 'Sarah Jenkins, LCSW',
            case_manager_phone: '(404) 657-5500',
          },
        });
      }
    } catch (err) {
      console.error('Failed to load client portal via Axios:', err);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleDownloadPacket = async () => {
    ...
    try {
      const stateCode = client?.service_address?.state || 'GA';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in text-white">
      ...
      <DashboardPageHeader
        ...
        subtitle={`Medicaid ID: ${client?.medicaid_id || 'Pending'} • DOB: ${client?.dob || 'N/A'} • Georgia CCSP / SOURCE Home Care Services`}
        icon={Heart}
        roleBadge={{ text: 'Client & Family Portal', variant: 'rose' }}
        stateBadge={{ code: client?.service_address?.state || 'GA' }}
        statusPill={{
          ...
          <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl space-y-4">
            ...
            <div className="space-y-3">
              <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs space-y-1">
                ...
                <span className="text-white font-medium block">{client.service_address?.street || '1428 Peachtree St NE'}</span>
                <span className="text-slate-300 block">{client.service_address?.city || 'Atlanta'}, {client.service_address?.state || 'GA'} {client.service_address?.zip || '30309'}</span>
              </div>
        ...
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl space-y-4">
          ...
          <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-2">
            <div className="flex justify-between items-center text-slate-300">
              ...
              <span className="font-bold text-white">{client?.payer_details?.program_name || 'Medicaid Home & Community-Based Waiver'}</span>
            </div>
            <div className="flex justify-between items-center text-slate-300">
            ...
            <div className="flex justify-between items-center text-slate-300">
              ...
              <span className="font-mono text-teal-400">{client?.payer_details?.case_manager_phone || '(404) 555-0100'}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'messages' && (
        <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold uppercase tracking-wider text-white">Agency Care Coordinator Contact</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-1">
              ...
              <span className="text-teal-400 font-mono font-medium block pt-1">(404) 555-0199</span>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-1">
              ...
              <span className="text-rose-400 font-mono font-medium block pt-1">(404) 555-0911</span>
            </div>
          </div>
        </div>
      )}
