...
interface GeorgiaTrainingClientProps {
  orgId: string;
  stateCode: 'GA' | 'IN' | 'FL';
}

export function GeorgiaTrainingClient({ stateCode }: GeorgiaTrainingClientProps) {
  ...
  useEffect(() => {
    let storedId = localStorage.getItem('crystal_caregiver_guest_id_ga');
    if (!storedId) {
      storedId = `ga-applicant-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
      localStorage.setItem('crystal_caregiver_guest_id_ga', storedId);
    }
  ...
  if (loading) {
    return (
      <div className="py-16 text-center text-slate-500">
        ...
        <p className="text-sm">Loading Georgia In-Service Training Portal...</p>
      </div>
    );
  }
    ...
    <>
      ...
      {activeCertHash && (
        <CertificateModal
          ...
          moduleTitle="Mandatory Georgia Home Care Training"
          hoursCredited={1.5}
          completedAt={new Date().toISOString()}
          certificateHash={activeCertHash}
          organizationName="With Open Hands Healthcare"
          stateCode="GA"
        />
      )}
    </>
