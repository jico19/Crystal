...
export const OrgThemeProvider: React.FC<OrgThemeProviderProps> = ({ children }) => {
  ...
  if (error && !org) {
    return (
      <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-6">
        <div className="max-w-md w-full bg-slate-800 border border-slate-700 rounded-lg p-6 text-center space-y-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded text-xs font-semibold uppercase tracking-wider transition-colors"
          >
            Retry
          </button>
        </div>
